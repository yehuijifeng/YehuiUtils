...
public class SuperSwipeRefreshLayout extends ViewGroup {
    ...
    public void setFooterView(View child) {
        if (child == null) {
            emptyLayoutParams = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, 0);
            mFooterViewContainer.setLayoutParams(emptyLayoutParams);
            return;
        }
    ...
    protected void onLayout(boolean changed, int left, int top, int right,
        ...
        //LogUtil.i("debug:onLayout childHeight = " + childHeight);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isChildScrollToBottom()) {
                        ...
                        if (yDiff > mTouchSlop && !mIsBeingDragged) {// 判断是否下拉的距离足够
                            ...
                            //下拉控件的拉伸上都，不将ontoush事件传入子控件，即截断listview的ontoush事件
    ...
    private boolean handlerPullTouchEvent(MotionEvent ev, int action) {
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                //LogUtil.i("y:"+y+"     overscrollTop"+overscrollTop+"");
                if (mIsBeingDragged) {
    ...
    private boolean handlerPushTouchEvent(MotionEvent ev, int action) {
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                mActivePointerId = MotionEventCompat.getPointerId(ev, 0);
                mIsBeingDragged = false;
                LogUtil.i("debug:onTouchEvent ACTION_DOWN");
                break;
            case MotionEvent.ACTION_MOVE: {
                final int pointerIndex = MotionEventCompat.findPointerIndex(ev,
                        mActivePointerId);
