package com.alsfox.chatlibrary.widget;
...
public class EaseChatInputMenuX extends LinearLayout {
	...
	public EaseChatInputMenuX(Context context, AttributeSet attrs, int defStyle) {
		this(context, attrs);
	}

	public EaseChatInputMenuX(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context, attrs);
	}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		layoutInflater.inflate(R.layout.easex_widget_chat_input_menu, this);
		...
		chatExtendPromptContainer = (FrameLayout) findViewById(R.id.extend_prompt_container);

		// 扩展按钮栏
		chatExtendMenu = (EaseChatExtendMenu) findViewById(R.id.extend_menu);
		// 扩展常用语
		chatExtendPrompt = (EaseChatExtendPrompt) findViewById(R.id.extend_prompt);
	}
	...
	public void init(List<EaseEmojiconGroupEntity> emojiconGroupList) {
		...
		if (chatPrimaryMenu == null) {
			chatPrimaryMenu = (EaseChatPrimaryMenuX) layoutInflater.inflate(R.layout.easex_layout_chat_primary_menu,
					null);
		}
	 ...
	 */
	public void setCustomPrimaryMenu(EaseChatPrimaryMenuBaseX customPrimaryMenu) {
	...
	public void setPromptEnable(boolean enable) {
		if (chatPrimaryMenu instanceof EaseChatPrimaryMenuX) {
			((EaseChatPrimaryMenuX) chatPrimaryMenu).setPromptEnable(enable);
		}
	}
	...
	public void registerExtendPromptItem(String name, int itemId, EaseChatExtendPrompt.EaseChatExtendPromptItemClickListener listener) {
		chatExtendPrompt.registerPromptItem(name, itemId, listener);
	}
	...
	public void registerExtendPromptItem(int nameRes, int itemId, EaseChatExtendPrompt.EaseChatExtendPromptItemClickListener listener) {
		chatExtendPrompt.registerPromptItem(nameRes, itemId, listener);
	}

	protected void processChatMenu() {
		// 发送消息栏
		chatPrimaryMenu.setChatPrimaryMenuListener(new EaseChatPrimaryMenuBaseX.EaseChatPrimaryMenuListenerX() {
		...
		if (chatExtendMenuContainer.getVisibility() == View.GONE) {
			...
			handler.postDelayed(new Runnable() {
				public void run() {
					...
					chatExtendPromptContainer.setVisibility(View.GONE);
				}
			}, 50);
		} else {
	...
	protected void toggleMoreNew() {
		if (chatExtendPromptContainer.getVisibility() == View.GONE) {
			hideKeyboard();
			handler.postDelayed(new Runnable() {
				public void run() {
					chatExtendPromptContainer.setVisibility(View.VISIBLE);
					chatExtendMenuContainer.setVisibility(View.GONE);
					chatExtendMenu.setVisibility(View.VISIBLE);
					emojiconMenu.setVisibility(View.GONE);
				}
	...
	public boolean onBackPressed() {
		if (chatExtendMenuContainer.getVisibility() == View.VISIBLE
				|| chatExtendPromptContainer.getVisibility() == View.VISIBLE) {
			hideExtendAllContainer();
			return false;
		} else {
			return true;
